...
def filter_low_abundance(
    ...
    return quantification_data.filter(pl.any_horizontal(filter_conditions)).drop(
        group_median_cols
    )
...
def filter_low_repeatability(
    ...
    for group in unique_groups:
        ...
        quantification_data = quantification_data.with_columns(
            [
                (null_count_expr / len(group_samples) <= max_missing_ratio).alias(
                    status_col_name
                )
            ]
        )
    ...
    return quantification_data.filter(pl.any_horizontal(group_status_cols)).drop(
        group_status_cols
    )
